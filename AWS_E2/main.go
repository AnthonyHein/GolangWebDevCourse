...
func main() {
	...
	db, err = sql.Open("mysql", "awsuser:mypassword@tcp(mydb.cnvvyo4x9krh.us-east-1.rds.amazonaws.com:3306)/golang?charset=utf8")
	check(err)
	defer db.Close()
...
func amigos(w http.ResponseWriter, req *http.Request) {
	rows, err := db.Query(`SELECT aName FROM golang.amigos;`)
	check(err)
